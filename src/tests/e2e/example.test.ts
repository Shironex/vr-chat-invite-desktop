...
test("renders the welcome page", async () => {
  ...
  expect(text).toContain("Welcome to Electron Starter Template");
});
...
test("renders features section", async () => {
  ...
  // Check for features card
  const featuresText = await page.textContent("body");
  expect(featuresText).toContain("React 19 with Compiler");
  expect(featuresText).toContain("TypeScript");
  expect(featuresText).toContain("shadcn-ui");
});
